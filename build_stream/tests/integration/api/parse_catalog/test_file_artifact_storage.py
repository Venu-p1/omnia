...
""")
    ...
    def test_parse_catalog_creates_artifacts_on_file_store(self) -> None:
        """Test that parse catalog creates artifact files on file store."""
        # Load a valid catalog from fixtures
        catalog_fixture_path = Path(__file__).parent.parent.parent.parent / "fixtures" / "catalogs" / "catalog_rhel.json"
        ...
        from core.jobs.value_objects import ClientId, IdempotencyKey
        ...
        import zipfile
        import io
        with zipfile.ZipFile(io.BytesIO(root_jsons_content), 'r') as zf:
